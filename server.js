...
// Cloudinary stores product images outside Render, so they survive redeploys/restarts.
cloudinary.config({
...
CREATE TABLE IF NOT EXISTS products(
 ...
 image_url TEXT DEFAULT '',stock INTEGER DEFAULT 0,active INTEGER DEFAULT 1,created_at TEXT DEFAULT CURRENT_TIMESTAMP);
...
if(!count){
 const add=db.prepare("INSERT INTO products(name,category,price,old_price,icon,image_url,stock) VALUES(?,?,?,?,?,?,?)");
 [
...
function uploadToCloudinary(file){
 return new Promise((resolve,reject)=>{
   if(!process.env.CLOUDINARY_CLOUD_NAME||!process.env.CLOUDINARY_API_KEY||!process.env.CLOUDINARY_API_SECRET){
     return reject(new Error("Cloudinary is not configured. Add CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY and CLOUDINARY_API_SECRET in Render Environment Variables."));
   }
   const stream=cloudinary.uploader.upload_stream({folder:"grameen-shop/products",resource_type:"image"},(error,result)=>error?reject(error):resolve(result.secure_url));
...
app.post("/api/admin/products",admin,upload.single("image"),async(req,res)=>{
 try{
   ...
   const r=db.prepare("INSERT INTO products(name,category,price,old_price,icon,image_url,stock) VALUES(?,?,?,?,?,?,?)")
     .run(name,category,price,old_price||0,icon||"🛍️",image_url,stock||0);
   res.json({ok:true,id:r.lastInsertRowid,image_url});
...
app.patch("/api/admin/products/:id",admin,upload.single("image"),async(req,res)=>{
 try{
   ...
   const x={...p,...req.body};
   let image_url=x.image_url||p.image_url||"";
   if(req.file) image_url=await uploadToCloudinary(req.file);
   db.prepare("UPDATE products SET name=?,category=?,price=?,old_price=?,icon=?,image_url=?,stock=?,active=? WHERE id=?")
    .run(x.name,x.category,x.price,x.old_price||0,x.icon||"🛍️",image_url,x.stock||0,x.active?1:0,req.params.id);
...
app.use((err,req,res,next)=>{
 if(err instanceof multer.MulterError)return res.status(400).json({error:err.code==="LIMIT_FILE_SIZE"?"Image must be 5MB or smaller.":err.message});
